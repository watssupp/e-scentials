...
function generate(document, schema, outputPath, only, target, tagName, nextToSources, options) {
    ...
    if (target === "swift") {
        ...
        const generator = (0, apollo_codegen_swift_1.generateSource)(context, outputIndividualFiles, suppressSwiftMultilineStringLiterals, only);
        ...
        const context = (0, compiler_1.compileToIR)(schema, document, options);
        const { generatedFiles, common } = (0, apollo_codegen_flow_1.generateSource)(context);
        const outFiles = {};
        if (nextToSources) {
        ...
        const context = (0, compiler_1.compileToIR)(schema, document, options);
        const generatedFiles = (0, apollo_codegen_typescript_1.generateLocalSource)(context);
        const generatedGlobalFile = (0, apollo_codegen_typescript_1.generateGlobalSource)(context);
        const outFiles = {};
        if (nextToSources ||
            ...
            const globalSourcePath = options.globalTypesFile ||
                path_1.default.join(outputPath, `globalTypes.${options.tsFileExtension || helpers_1.DEFAULT_FILE_EXTENSION}`);
        ...
        let output;
        const context = (0, legacyIR_1.compileToLegacyIR)(schema, document, Object.assign(Object.assign({}, options), { exposeTypeNodes: target === "json-modern" }));
        switch (target) {
            case "json-modern":
            case "json":
                ...
                break;
            case "scala":
                output = (0, apollo_codegen_scala_1.generateSource)(context);
        }
        if (outputPath) {
