        ...
        const { validationResults, operations } = await this.runTasks(({ project, config }) => [
            {
                ...
                task: async (ctx) => {
                    ...
                    ctx.gitContext = await (0, git_1.gitInfo)(this.log);
                    ctx.operations = Object.entries(this.project.mergedOperationsAndFragmentsForService).map(([name, doc]) => ({
                        body: (0, graphql_1.print)(doc),
                        name,
                        relativePath: (0, path_1.relative)(config.configURI ? config.configURI.fsPath : "", vscode_uri_1.default.parse(doc.definitions[0].loc.source.name).fsPath),
