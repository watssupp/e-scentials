                ...
                return [
                    {
                        ...
                        task: async (ctx, task) => {
                            const operationManifest = (0, getOperationManifestFromProject_1.getOperationManifestFromProject)(this.project);
                            ctx.operationManifest = operationManifest;
                            task.title = `Extracted ${(0, utils_1.pluralize)(operationManifest.length, "operation")} from client, ${clientBundleInfo}`;
                        },
                    },
                    {
                        ...
                        task: async (_, task) => {
                            ...
                            if (!registrationSuccess) {
                                if (invalidOperations) {
                                    ...
                                    task.title = `Failed to push operations, due to ${(0, utils_1.pluralize)(invalidOperations.length, "invalid operation")}`;
                                ...
                                if (newOperations && newOperations.length) {
                                    task.title = `Successfully pushed ${(0, utils_1.pluralize)(newOperations.length, "operation")} to the operation registry`;
                                    result += (0, table_1.table)([
...
function generateSignatureToOperationMap(project, config) {
    return Object.fromEntries(Object.entries(project.mergedOperationsAndFragmentsForService).map(([operationName, document]) => {
        ...
        const relativePath = operationDefinition &&
            ...
            (0, path_1.relative)(config.configURI ? config.configURI.fsPath : "", vscode_uri_1.default.parse(operationDefinition.loc.source.name).fsPath);
        const line = operationDefinition &&
            operationDefinition.loc &&
            operationDefinition.loc.source.locationOffset.line;
        return [
            (0, apollo_graphql_1.operationHash)((0, apollo_graphql_1.defaultOperationRegistrySignature)(document, operationName)),
            {
                operationName,
                document,
